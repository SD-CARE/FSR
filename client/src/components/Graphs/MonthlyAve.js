...
import MonthlyBarMetric from "./MonthlyBarMetric";
import { Context } from "../../Context";
export const MonthlyAve = React.forwardRef((props, ref) => {
  ChartJS.register(
    CategoryScale,
    LinearScale,
    PointElement,
  ...
  //  group all ratings by carerID and month in an array
      ...
      ? setCarerRatingsGrouped(
          carerRatings.reduce((r, a) => {
            ...
            if (
              ...
              r[new Date(a.startDate).getMonth() + 1] =
                r[new Date(a.startDate).getMonth() + 1] || [];
              r[new Date(a.startDate).getMonth() + 1].push(a);
  ...
  useEffect(() => {
    if (metricRating) {
      setStartDate(metricRating.map((rate) => rate.startDate));
    }
  }, [metricRating]);

  let filteredStartDate = new Set(startDate);
