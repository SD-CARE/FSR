...
function Carers() {
  ...
  const id = carers ? carers.map((carer) => carer.carerID) : [];
        ...
        {allCarers
          ...
          .map((carer, index) => (
            <form key={index}>
              <Link
                to={`/carers/${
                  cpid.includes(carer.identifier)
                    ? id.indexOf(cpid.indexOf(carer.identifier)) + 2 + "/date"
                    : `NPC/create/${carer.identifier}`
                }`}
                className={`carer--module ${
                  cpid.includes(carer.identifier) ? "active" : "not-active"
                }`}
              >
